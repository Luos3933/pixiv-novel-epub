...
pixiv-novel quick <系列ID或网址>                  # 一键下载、标准化并以书名生成 TXT + EPUB

# 下载（旧脚本入口；pixiv_novel_scraper.py 直接运行会委托给 cli.py）
python cli.py series <系列ID或网址>              # 下载整个系列（支持完整/省略 https:// 的 Pixiv 系列网址）
...
python cli.py novel <小说ID或网址> --chapter <编号> --force
...
python txt_file_processing.py toc inspect <章节目录或整本txt> [输出文件] # 纯检查目录：按真实编号排序、保留重复号、插入缺号占位行
...
- **目录导出/回写/检查**（`toc` 子命令，`TocManager` 类）：`export` 目录模式输出 `NNN 标题`（默认 `<目录>/_toc.txt`）、整本 txt 模式用与 split 相同识别规则预览标记+缺口分析（默认 `<同名>_toc.txt`，番外/无编号标记给顺延对照号而非注释——保持 apply 条目数一致），自动附缺口/重复注释；`inspect` 输出不可回写的纯检查目录（默认 `_toc_inspect.txt`/`<同名>_toc_inspect.txt`），按真实编号稳定排序，重复号全部保留并在行尾按 `【重复 1/2】` 标记，最小到最大编号间的缺号写成纯数字占位行，标题统一为「第中文数字章 章名」，目录输入优先正文首个非空标题行、其次文件名标题、最后文件名前缀，无编号番外忽略并告警；`apply` 目录模式按前缀找文件重命名+替换首行（首行是标记行或与旧文件名标题一致时）、整本 txt 模式按顺序替换标记行（首次 `shutil.copy2` 生成 .bak，条目数与标记数不符中止）；TOC 格式 `^(\d{1,4})\s+(.+)$`，读取走编码自动探测（utf-8-sig 兼容 BOM、记事本 ANSI），`#` 注释行忽略——用于人工清理「爆更/(4爆)/+上架」等连载标注
