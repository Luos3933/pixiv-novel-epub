...
把「一卷一个文件、卷内章节未划分」的原始下载拆成独立章节（识别 `第X章/番外/纯数字` 标记行、按顺序修正作者编号错误、全局连续编号）；同时在上级目录生成 `volumes.json`（锚定每卷全局章节范围，供 `--volumes` 用），输出目录自动生成 `000 书籍信息.txt` 与 **`_编号统计.txt`（缺失/重复编号清单）**。也可以直接传**整本 txt 文件路径**（视为单卷，外来网文常见形态）。**外来 txt 编码自动探测**（UTF-8/GBK/Big5），输出统一 UTF-8：

```bash
python txt_file_processing.py split <原始目录或整本txt> <输出目录> [--punct] [--name-only] [--title-len-limit] [--unify-title] [--num-style chinese|arabic] [--no-renumber] [--max-gap N]
```
...
- `--unify-title`：标题统一为「第X章 章名」（编号转中文数字、冒号/粘连归一为空格、纯数字编号 `407：xxx` / `588xxx` 补「第/章」式）；适合章节号类型混杂（`第一章：异世界` / `407：章` / `588对战阿法摩` 混排）的外来整本 txt
- `--num-style`：`--unify-title` 时的编号风格，`chinese`（第一章，默认）/ `arabic`（第407章）
- `--no-renumber`：关闭编号重排，保留作者原编号；导出文件名前缀沿用原编号，**编号缺口即漏章线索**（缺口/重复在日志报告）；默认按出现顺序重排为连续编号
- `--max-gap N`：章节编号**连续性校验**上限——新识别编号比最高编号**回退**超过 N 章的标记行不视为章节（该行并入上一章正文，并记入 `_编号统计.txt` 的「被连续性校验拒绝」清单供人工核对），防正文短行（如「005般的几个同学……」）在第 475 章附近被误判成「跳回第 5 章」；**前向跳变不校验**（断更/漏发造成的缺号块属正常，由缺失清单报告）；**同号同题**（归一化去标点后一致）的作者重发放行；`0` 关闭校验。默认取 `split_config.json` 的 `chapter_gap_limit`（10），显式参数优先
- **拆分配置文件 `split_config.json`**（项目根目录，与 `epub_styles.json` 同模式）：存放拆分识别参数，缺失时自动生成带说明的默认模板——`chapter_gap_limit`（连续性校验上限，0/null 关闭）、`marker_max_len`（标记行最大长度，默认 40）

**`_编号统计.txt`**（每次 split 自动生成到输出目录，两种模式都有）：按章节标记的**原编号**统计——缺失编号（压缩范围，如 `433-434、443`，原文缺号或有标记未识别）、重复编号（作者标号错误，逐个列出对应导出文件，标明原编号/顺延）、起始编号提示（首个编号非 001 时）、疑似未识别标记清单（整本 txt 模式：数字开头、编号在已识别范围内但形式未匹配的行，含行号，供人工核对本章是否真实存在）、被连续性校验拒绝的疑似标记（编号回退超过上限的行，含行号，已并入上一章正文）。重排模式下导出编号连续，但原文的断号与错标仍在此文件可见。`_` 开头文件为系统生成文件，merge/epub/toc 等下游命令均自动跳过。

**识别的章节标记**（中文数字支持逐位风格 `第一四四章` = 144）：
...
把全部章节目录导出成一个文件，人工清理标题里的「爆更/更新/+上架/(4爆)」等连载标注后回写。**拆分前**（整本 txt：预览标记识别+缺口分析）和**标准化后**（章节目录：按编号重命名/改首行）两个时机都支持：
...
- 目录模式 apply：按编号找到文件 → 重命名（`os.replace`）→ 首行是标记行或与旧文件名标题一致时同步替换首行
...
- **适用场景**：pixiv 系列中「一卷一个文件、卷内全部章节未划分」（如 series_123456789 每文件含 20+ 章），或外来整本 txt 单文件（直接传文件路径）
- **识别规则**：独立成行（≤40 字）的 `第X章/第X话/第X节/第X回`（中文/阿拉伯数字，支持逐位风格 `第一四四章`=144，编号后跟空格/冒号/行尾，或**无空格但不含句号**的短标题如「第二章天使降临我身边？」；含句号视为正文）+ `番外：xxx`/`番外 xxx` + 纯数字编号 `407章`/`407：章`/`588对战阿法摩`（粘连靠不含句号+排除年月日/数量词起始行防误判）；`--title-len-limit` 可开启 ≤20 字严格模式（默认关闭，兼容长标题网文）
- **编号修正**：按出现顺序重排作者标号错误（重复/回退），以首个标记编号为起点；中文风格保持中文、阿拉伯保持阿拉伯、纯数字补「第/章」式；番外不占编号；`--no-renumber` 关闭重排保留原编号（缺口即漏章线索）
- **编号连续性校验**（`split_config.json` 的 `chapter_gap_limit`，默认 10，`--max-gap` 可临时覆盖）：新识别编号比**最高已识别编号**回退超过上限的标记行不视为章节，并入上一章正文并记入 `_编号统计.txt`；前向跳变不校验（缺号块正常）；同号同题的作者重发放行（如重发的重复章）；0/null 关闭。标记行长度上限 `marker_max_len`（默认 40）同样在此配置
- **标题统一**（`--unify-title`）：混杂标记统一为「第X章 章名」（默认中文数字，`--num-style arabic` 可选），冒号/粘连归一为空格
