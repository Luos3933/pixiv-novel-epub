...
- 新增 `clean <目录或txt> [输出目录]`：按 `text_processing.json` 中逐项开关的安全规则清洗正文，默认写入输入同级 `cleaned/`，原始文件保持不动；支持 `--dry-run` 仅生成预览报告。
- 新增 `audit <目录或txt> [报告目录]`：检查解码损坏、引号配对、长段落、相邻重复行、近距离重复章节、广告关键词、疑似硬回车和标点异常，默认在 `reports/` 同时生成便于阅读的 TXT 与结构化 JSON。
- 新增统一 `text_processing.json`，配置清洗/检查规则、阈值、关键词与默认目录；配置缺失时自动生成完整模板，非法单项独立回退默认值。
- `clean` 新增保守的作者附言块清理：先同时命中 `PS` / `P.S.` / `作者的话` 行首和求票/收藏关键词，再继续删除有限数量、命中推荐新书/求票/收藏信号的续段，遇正常正文立即停止；报告同时列出启用/停用规则、按规则修改统计和输出目录遗留 TXT。
- `clean` / `audit` 新增连续空行章末附言规则：检查章末至少 2 个连续空行后的顶格作者说明，只有开头命中赞助地址、交流群、感谢支持、荐书、插画说明或求票等强信号才报告/删除；两个全角空格开头的中文正文保留，clean 启用时同时删除尾块中的 `【插图: 文件名】` 文本标记但不删除图片文件，后续 EPUB 可按插图信息与文件名章节号还原。clean 删除开关默认关闭，支持在外来整本 TXT 拆分前运行。
- `clean` 新增默认关闭的连续空行场景跳转规则：启用后把不少于 3 个连续空行替换成独立 `……` 段，相邻已有独立省略号时只规范空行而不重复插入，并排除章末作者附言候选。两项连续空行规则无论是否启用，均分别输出 TXT/JSON 候选报告；场景报告包含前后各 2 段。
- 正常 `clean` 完成后自动创建或复用同级 `reviewed/`，不覆盖已有人工复核稿；`--dry-run` 不创建目录。`format` 会自动叠加输入同级 `reviewed/`，也支持 `--reviewed-dir <目录>` 显式指定未经过 clean 的格式化前人工修订层，同编号人工文件优先。

#### 修复

- `format --punct` 改为整章保持双引号配对状态，跨行英文对话不再把闭引号误转为第二个开引号。
- 编码探测在 UTF-8/GB18030/Big5 均含少量坏字节时，不再无条件误判为 GB18030；按正文特征选择最可信编码、用替换符保留其余内容，并在清洗/检查报告中定位损坏文件。
- `audit` 目录模式不再把输出目录中遗留的无数字前缀整本 TXT 与分章文件重复检查；分章文件的章节标题异常只检查首个非空行，避免把正文中的数字开头句误报成标题。

#### 验证

- 新增配置、清洗、审计、独立输出目录、dry-run、容错解码、CLI 编排、reviewed 覆盖及跨行引号回归测试；全量 154 项自动化测试通过。
...
- **编号连续性校验**（`VolumeSplitter._split_file`，`chapter_gap_limit` 默认 10）：新识别编号比**最高已识别编号回退超过上限**的标记行不视为章节——该行并入上一章正文（内容不丢），记入 `_编号统计.txt` 新增的「被连续性校验拒绝的疑似标记」清单（含行号），并按正文行处理；防正文短行（如「005般的几个同学……」）在第 475 章附近被误判成「跳回第 5 章」产生假章。设计要点：
  - **前向跳变不校验**——断更/漏发造成的缺号块（如 424 之后缺 425~434 直接跳 435，前向差距 11）是正常现象，由缺失编号清单报告；若按绝对差距校验会从缺号块处开始级联拒绝其后全部章节（真实数据实测 269 行被拒，故只校验回退）
  - **同号同题重发豁免**（`_norm_title_key` 归一化去空白/标点）：大幅回退但与之前同编号同标题的标记判定为作者重发（重复连载常见形态），正常放行并交由重复编号机制顺延导出号——真实数据中 657 章后逐字重发的 604 章靠此豁免；回退幅度以**最高编号**为基准（作者重发后继续发新章时不会被重发章拉低的基准漏判）
  - 每个卷文件独立校验（按文件重置基准，兼容按卷重新编号的书）；番外/无编号标记不参与
- **拆分配置文件 `split_config.json`**（项目根目录，与 `epub_styles.json` 同模式，`_load_split_config` 读取）：存放拆分识别的初始设置，缺失时自动生成带 `_说明` 的默认模板；单键非法（非非负整数）回退默认值并告警。键：`chapter_gap_limit`（连续性校验上限，0/null 关闭）、`marker_max_len`（章节标记行最大长度，默认 40，`_parse_marker` 长度判定改用此值）
- **`split --max-gap N`**：临时覆盖配置文件的连续性校验上限（`0` 关闭校验），方便单次跑不同口径

#### 修复

- 真实数据（外来整本 GBK txt）：正文行「005般的几个同学……」在第 475 章附近被误判为第五章标记，`--no-renumber` 下占用导出号 476 导致其后全部章节文件名整体 +1 错位；加连续性校验后重拆，700 章（原 702 − 假章 − 并回正文的内部编号头），导出号恢复与原文编号对齐
- `_编号统计.txt`：被连续性校验拒绝的行不再重复出现在「疑似未识别标记」清单中

#### 验证

- 场景断言全过：大幅回退误判行拒绝并并入上一章正文、前向缺号跳变（476→489）放行、同号同题大幅回退重发放行（顺延导出）、同号不同题大幅回退拒绝、`--max-gap 0` 关闭校验、配置文件 `chapter_gap_limit: 0` 关闭校验、配置文件缺失自动生成、重排（renumber）模式下同样受保护
- `python -m py_compile` 通过；`split --help` 新参数注册成功
- 真实文件重拆验证：700 章、缺失 23、重复 17 处，被拒清单恰好 2 行（正文粘连行 + 654 章内部旧编号头「605 不负责」，后者因原 605 从未单独存在无法同号同题豁免，正文完整并入 654）；`_重复与空章.txt` 按新导出号重新生成（空壳 18、重复 17 组 = 空壳+正文 11、纯重发 1、两版不同 5）

### v0.9.0 - 2026-08-31

针对**外来整本 txt 小说**的拆卷格式化增强：章节号类型混杂归一、章节目录导出/回写、编号重排开关。

#### 新增

- **外来 txt 编码自动探测**（`_detect_encoding`/`_read_text_lines`/`_read_text_auto`）：BOM 优先 → 严格 UTF-8 → gb18030/big5 按常见汉字占比择优（简繁混收参考集）；split 与 toc 的整本 txt、章节文件、目录文件读取全部接入，非 UTF-8 时 INFO 告知；输出仍统一写 UTF-8（整本 txt 回写前 `.bak` 保留原文件原编码）
- **逐位中文数字**：`_chinese_to_int` 支持「第一四四章」= 144（旧逻辑误算成 4，不超过 4 位防长串误读）
- **纯数字编号识别**：`VolumeSplitter` 新增 `407章 xxx`（bare_sfx）/ `407：XX` / `437 XXX` / `588XXXX`（bare）三类外来网文常见标记；无分隔粘连靠「不含句号」判定，`GLUED_BLOCK_CHARS` 排除年月日/数量词起始行防正文误判；**空标题纯编号 `401：` 也识别**（标题用「第X章」占位，unify 后即「第四百零一章」）；纯数字单独成行仍不识别（防正文数字行误判）
- **`_编号统计.txt`**（split 自动生成到输出目录，重排/保留原编号两种模式都有）：按章节标记**原编号**统计——缺失编号压缩范围（`433-434、443`）、重复编号逐条列出对应导出文件（标明原编号/顺延分配）、起始编号非 001 提示、**疑似未识别标记清单**（整本 txt 模式：数字开头、编号落在已识别范围内但形式未匹配的行——粘连标题首字在防误判字符集、标题与正文同行、纯数字单独成行等；数量词/日期起始且呈正文形态的长行排除；上限 100 条），方便人工排查漏章与作者错标
- **`_` 前缀系统文件约定**：`_toc.txt`/`_编号统计.txt`/`_source_map.txt` 等工具生成文件，merge 合并/字数统计、epub 章节收集、format 输入、diff/note/assemble 配对（`_list_txt_in`/`_build_prefix_index`）一律跳过——顺带修复 `_toc.txt` 可能被当作无前缀章节混入 merge/epub 的隐患
- **`split --unify-title`**：混杂标记统一为「第X章 章名」（`--num-style chinese|arabic`，默认 chinese），冒号/粘连归一为空格
- **`split --no-renumber`**：关闭编号重排保留作者原编号；导出文件名前缀沿用原编号（`_alloc_out_num` 冲突顺延），编号缺口/重复/卷范围重叠在日志报告，返回值新增 `gaps` 键——**缺口即漏章线索**
- **`toc` 子命令**（新 `TocManager` 类）：
  - `toc export <目录或整本txt> [输出文件]`：目录模式输出 `NNN 标题` 列表（默认 `<目录>/_toc.txt`）；整本 txt 模式预览标记识别+缺口分析（默认 `<同名>_toc.txt`）；自动附缺口/重复注释行
  - `toc apply <路径> <目录文件>`：目录模式按编号重命名+替换首行；整本 txt 模式按顺序替换标记行（首次自动 `.bak` 备份，条目数不符中止防错位）
  - 覆盖「标准化前清理整本 txt 标题」与「标准化后清理章节目录」两个时机，用于去除「爆更/更新/+上架/(4爆)」等连载标注
- **整本 txt 单文件输入**：`split <整本.txt> <输出目录>` 视为单卷拆分
- `_parse_marker` 返回结构化 dict `{raw, num, style, suffix, sep, title}`（style 新增 `bare`/`bare_sfx`），epub `_resolve_chapter_title` 沿用同一真值判断

#### 修复

- `FANWAI_RE` 扩展支持空格分隔「番外 xxx」（与旧 docstring 声明及 epub `_split_title` 行为对齐）；番外分支加句号排除防正文行误判
- `toc export` 整本 txt 模式：番外/无编号标记从 `#` 注释改为带顺延对照号的正常行（否则 apply 条目数与标记数不符会中止，导出文件无法直接编辑回写）

#### 验证

- `.tmp_v090_test.py` 56 项断言全过：`_chinese_to_int` 逐位数字、`_parse_marker` 14 正例 + 7 反例（正文误判防护）、split 单文件 unify-title、no-renumber 原编号导出与缺口报告、toc export/apply 整本 txt 与目录双模式、pixiv 风格回归（编号修正/粘连补空格/冒号保留/volumes.json/epub 首行解析）
- `python -m py_compile` 四模块通过；`split`/`toc`/`toc export`/`toc apply --help` 注册成功
- 真实数据误判扫描：`series/*/chapters` 118 文件 + standardized/corrected/final 各层首行，bare 类零误报；唯一 fanwai 命中（`044 番外：晟朝的人们 一`）为真实番外且旧正则本就识别，非新增误报
- 编码闭环测试：GBK 整本（首字节 0xA1）探测为 gb18030、split/unify-title/no-renumber 输出 UTF-8；Big5 探测正确；toc export/apply 在 GBK 文件上闭环（编辑目录存 ANSI 也能回写，`.bak` 生成）；UTF-8 输入回归无影响
- `401：` 空标题识别与 `_编号统计.txt` 专项测试：空标题占位（unify 后「第四百零一章」）、缺失范围压缩、重复编号逐条列出（标明原编号/顺延）、`_` 前缀系统文件在 `_build_prefix_index` 中被跳过、`_compress_ranges` 边界（单值/多段）
- 真实文件验证：外来整本 txt（GBK，704 章连载）`--unify-title --no-renumber` 最终拆出 702 章，`_编号统计.txt` 列出 18 处重复编号、22 个缺失编号（均为原文真缺号）——首轮 687 章 + 37 缺失，经疑似清单（15 条）定位漏识别标记后按行修复原文（补空格/改「第X章 」前缀/补冒号/标题正文拆行，修复前自动备份）再重跑；`volumes.json` 与 `000 书籍信息.txt` 正常生成
